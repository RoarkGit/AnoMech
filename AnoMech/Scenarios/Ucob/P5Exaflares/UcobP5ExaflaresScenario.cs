...
public sealed class UcobP5ExaflaresScenario : IScenario
{
    ...
    // The whole mechanic lives on the private `timeline`, so Game's default IsFinished
    // (world.Events.IsEmpty) would read true from the first tick. Watch the real queue instead.
